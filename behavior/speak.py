...
class OrrinSpeaker:
    ...
    def recall_connection_marker(self, thought):
        try:
            private_thoughts = filter_memories(load_private_thoughts_as_list(PRIVATE_THOUGHTS_FILE), tag="[recall_connection_marker:private_thoughts]")
            long_memory = filter_memories(load_json(LONG_MEMORY_FILE, default_type=list), tag="[recall_connection_marker:long_memory]")
    ...
    def rephrase_with_tone(self, thought, tone_data, context):
        ...
        recent_tones = [entry.get("tone", "neutral") for entry in self.conversation_history[-4:] if isinstance(entry, dict)]
...
# (You can now safely merge/filter these anywhere in your pipeline.)
